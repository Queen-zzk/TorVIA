...
static inline int get_SH_version(uint8_t *lp_buf, int len_buf)
{
    int iout = 2;

    int pos;
    if(lp_buf[0] == 0x16 && lp_buf[1] == 3 && lp_buf[2] == 3 && lp_buf[5] == 2)//server hello
    {
        int len_sh = lp_buf[3] * 256 + lp_buf[4];
        pos = 43;
        if(len_buf > pos + 1 && len_sh > pos + 1)
        {
            int len_sid = lp_buf[pos];
            pos = pos + 1 + len_sid + 2;
            if(len_buf > pos + 1 && len_sh > pos + 1)
            {
                int len_cps = lp_buf[pos];
                int pos_ex = pos + 1 + len_cps + 2;

                pos = pos_ex;
                while(len_buf > pos + 4 && len_sh > pos + 4)
                {
                    if(lp_buf[pos] == 0 && lp_buf[pos+1] == 43)     //support version
                    {
                        if(len_buf >= pos + 6 && len_sh >= pos + 6)
                        {
                            if(lp_buf[pos+2] == 0 && lp_buf[pos+3] == 2 && lp_buf[pos+4] == 3 && lp_buf[pos+5] == 4)
                            {
                                iout = 3;
                                break;
                            }
                    ...
                    {
                        int len_ext = lp_buf[pos+2] * 256 + lp_buf[pos+3];
                        pos += 4 + len_ext;
                    }
                }
            }
